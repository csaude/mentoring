...
public interface FormQuestionDAO extends GenericDAO<FormQuestion, Long> {
	...
	class QUERY {
		...
		public static final String fetchByForm = "SELECT fq FROM FormQuestion fq INNER JOIN FETCH fq.form f INNER JOIN FETCH f.programmaticArea INNER JOIN FETCH fq.question q WHERE f.id = :formId AND fq.lifeCycleStatus = :lifeCycleStatus";
	}
