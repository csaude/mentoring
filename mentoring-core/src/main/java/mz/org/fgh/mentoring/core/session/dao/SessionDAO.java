...
public interface SessionDAO extends GenericDAO<Session, Long> {
	...
	class QUERY {
		...
		public static final String findWithDuplicatedUuids = "SELECT s FROM Session s GROUP BY s.uuid HAVING COUNT(s.uuid) > 1 AND s.lifeCycleStatus = :lifeCycleStatus";
