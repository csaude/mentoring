...
public class CarrerDAOImpl extends GenericDAOImpl<Career, Long> implements CareerDAO {
	...
	@Override
	public Career findByTypeAndPosition(final CareerType careerType, final String position,
	        final LifeCycleStatus lifeCycleStatus) throws BusinessException {
		return this.findSingleByNamedQuery(CareerDAO.QUERY_NAME.findByTypeAndPosition,
		        new ParamBuilder().add("careerType", careerType).add("position", position)
