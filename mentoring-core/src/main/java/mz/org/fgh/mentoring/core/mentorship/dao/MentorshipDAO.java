...
public interface MentorshipDAO extends GenericDAO<Mentorship, Long> {
	...
	class QUERY {
		public static final String findNumberOfSessionsPerHealthFacility = "SELECT hf.HEALTH_FACILITY, COUNT(m.ID) as TOTAL, MAX(m.CREATED_AT) as LAST_UPDATE FROM MENTORSHIPS m INNER JOIN HEALTH_FACILITIES hf on hf.ID = m.HEALTH_FACILITY_ID WHERE m.LIFE_CYCLE_STATUS = :lifeCycleStatus GROUP BY hf.HEALTH_FACILITY ORDER BY hf.HEALTH_FACILITY";
	}
